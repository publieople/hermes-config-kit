...
## 常见格式错误（docs +create / +update 的 markdown 陷阱）

| 错误模式 | 现象 | 修复 |
|---------|------|------|
| Callout 内嵌代码块（````` ``` ````） | 报错 "Callout does not support Code block type" | 把代码块移到 Callout 外面。Callout 只支持文本、列表、标题、待办、引用 |
| Callout 内嵌表格 | 静默丢弃或报错 | 表格移到 Callout 外部 |
| Markdown 表格中内容含格式化语法（加粗、代码等） | 解析失败 | 改用 `<lark-table>` 增强表格语法 |
| `--format json` 用于 shortcut 命令 | 报 "unknown flag: --format" | shortcut（`+create`、`+search` 等）默认输出 json，删掉 `--format json` 参数 |
| 长 markdown 内容直接用 shell 变量传参 | shell 长度限制或转义问题 | 先写入 .md 文件，用 `--markdown "$(cat file.md)"` 传入。仍失败则分段创建（先 +create 骨架，再 +update --mode append 追加段落）
