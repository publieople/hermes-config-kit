...
**⚠️ 视觉工具选择（重要）：**
- 优先用 `vision_analyze`（Hermes 内置视觉）
- 若当前辅助视觉模型不支持图像（如 MiniMax M2.7 是纯文本模型），**改用 `mmx vision describe <image_path>`**（通过 mmx-cli 调用 MiniMax VLM 端点）
...
- **特殊：还款类账单**（白条还款等定期还款）——数据库中可能已有记录（按还款日创建），截图中的扣款可能是同一条的银行端体现，按用户偏好处理
...
**常见更新场景：白条还款**\n- 数据库中已有按还款日创建的白条还款记录（无渠道、未结算）\n- 收到银行账单截图后，将日期改为银行实际扣款日\n- 添加交易渠道为扣款卡（如中国银行储蓄卡）\n- 状态改为「已结算」\n- 注意：还款类账单默认不需要查，但用户主动要求调整时按此流程处理\n\n**银行端显示名对照**\n京东白条还款在银行账单中可能显示为"网银在线-肯特瑞小额贷..."。遇到此类银行端扣款时，记录为：\n- 项目名：「白条还款」\n- 大项=日常购物，小项=还款，标签=还款
...
| 余额宝收益 | 收入/利息 | 利息 | 渠道=余额宝，标签=利息 | 余额宝转入转出差额（如 0.36） |
